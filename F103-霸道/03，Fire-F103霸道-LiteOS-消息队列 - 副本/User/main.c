...
static void Receive_Task(void)
{
	...
	while(1)
	{
		buffsize = TEST_QUEUE_SIZE;
		
		/* 队列读取（接收），等待时间为一直等待 */
		uwRet = LOS_QueueReadCopy(Test_Queue_Handle,	/* 读取（接收）队列的ID(句柄) */
													r_queue,			
												  &buffsize,		
...
static void Send_Task(void)
{
	...
	while(1)
	{
		...
		if( Key_Scan(KEY1_GPIO_PORT,KEY1_GPIO_PIN) == KEY_ON )      
		{
			...
			uwRet = LOS_QueueWriteCopy(	Test_Queue_Handle,	/* 写入（发送）队列的ID(句柄) */
